...
module AHBLITE_SYS(
    ...
    input  wire        RsRx,              // <-- M?I: UART Receive
    output wire        RsTx,              // <-- M?I: UART Transmit 
    ...
    // TO LCD
    output wire             LCD_RS_OUT,
    output wire             LCD_RW_OUT,
    output wire             LCD_E_OUT,
    output wire    [3:0]    LCD_DB_OUT
...
//    );
  AHBUART uAHBUART (
      ...
      .HWDATA(hwdatas),
      .HWRITE(hwrites),
      .HREADY(hreadys),   // HREADY chung t? MUX
      
      .HREADYOUT(hready_uart), // HREADYOUT c?c b? t?i MUX
      .HRDATA(hrdata_uart),    // HRDATA c?c b? t?i MUX
      
      .HSEL(hsel_uart),   // T n hi?u ch?n t? Decoder
      
      //Serial Port Signals
      .RsRx(RsRx),      // N?i v?i c?ng c?p cao nh?t
      .RsTx(RsTx)      // N?i v?i c?ng c?p cao nh?t
